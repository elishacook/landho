...
module.exports = function (wss, options)
{
    ...
    var send_message = function (socket, name, data)
    {
    ...
    return function (api)
    {
        var setup = function (socket, user)
        {
            socket.on('message', function (raw_message)
            {
                ...
                {
                    send_message(socket, 'error', { code: 400, message: 'Could not parse message data' })
                    return
                }
                
                if (!message.name || !message.id)
                {
                    send_message(socket, 'error', { code: 400, message: 'Invalid message. A name and ID are required.' })
                    return
                }
